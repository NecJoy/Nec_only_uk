...
class FaqScreen extends StatelessWidget {
  final WebViewPageController _webviewPageController = Get.put(WebViewPageController());
  final key = UniqueKey();
              ...
              if (_webviewPageController.connected.value)
                WebView(
                  ...
                  onProgress: (int process)async{
                     if(process > 20){
                        _webviewPageController.removeHeaderForFaq();
                     }
                  },
                  onPageFinished: (String url) {
                    _webviewPageController.isLoading.value = false;
                    _webviewPageController.removeFotter();
                    _controller.runJavascriptReturningResult('Tawk_API.hideWidget();');
                  },
                ),
